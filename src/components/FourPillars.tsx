...
import { Scale, Feather, Equal, HeartHandshake, Landmark } from 'lucide-react';
...
export const FourPillars: React.FC = () => {
  const { t } = useTranslation('fourPillars');
  const [activePillar, setActivePillar] = useState<number>(0);
  const pillars = t('pillars', { returnObjects: true }) as Pillar[];
  const duties = t('duties', { returnObjects: true }) as string[];

  return (
    <section id="four-pillars" className="py-16 sm:py-24 bg-[#F8F6F0] text-[#0A1F44] border-b border-[#0A1F44]/10 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-5xl font-serif-display font-bold text-[#0A1F44] tracking-tight">
            {t('title')}
          </h2>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-[10px] font-mono ${isSelected ? 'opacity-60' : 'text-[#0A1F44]/50'}`}>{t('pillarLabel', { num: idx + 1 })}</span>
                </div>
                ...
                <div className={`mt-4 text-[11px] font-mono flex items-center justify-between ${isSelected ? 'text-[#FF9933]' : 'text-[#0A1F44] font-semibold'}`}>
                  ...
                  <span className="underline">{t('explore')}</span>
                </div>
              </button>
            );
          })}
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-white/15 pb-4 mb-4">
            <div>
              <div className="text-[10px] font-mono text-[#FF9933] uppercase tracking-[0.2em]">
                {t('analysisLabel', { num: activePillar + 1 })}
              </div>
            ...
            <div className="px-3 py-1 bg-white/10 border border-white/20 text-xs font-mono text-[#F8F6F0]">
              {t('articlesLabel', { articles: pillars[activePillar].articles })}
            </div>
          </div>
